...
const mustShowLabels = (filter, hasTrelloCard, card) => {
  ...
  if (filter.indexOf('NO_LABEL') > -1 && card.labels.length === 0) return true;

  if (filter.some((filterLabel) => card.labels.some((cardLabel) => isEqual(cardLabel, filterLabel)))) return true;
...
const mustShowDue = (filter, hasTrelloCard, card) => {
  ...
  if (filter === 'WITH_INCOMPLETE_DUE' && card.due !== null && card.dueComplete === false) return true;

  if (filter === 'WITH_COMPLETE_DUE' && card.dueComplete === true) return true;

  if (filter === 'WITHOUT_DUE' && card.due === null) return true;
...
const mustShowLocations = (filter, hasTrelloCard, card) => {
  ...
  if (filter.some((filterLocation) => isEqual(card.location, filterLocation))) return true;

  return false;
};

const mustShowProcesses = (filter, hasTrelloCard) => {
  if (filter === 'WITHOUT_CARD' && !hasTrelloCard) return true;
